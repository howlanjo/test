...
void xmit_spi(BYTE dat)
{
    ...
    /* Send data. */
    SPI_transmitData(EUSCI_A3_MODULE, dat );	//was B0

    /* flush data read during the write */
    ui32RcvDat = SPI_receiveData(EUSCI_A3_MODULE);
...
BYTE rcvr_spi (void)
{
    ...
    /* Send a dummy. */
    SPI_transmitData(EUSCI_A3_MODULE, 0xFF);		//Was B0

    ui32RcvDat = SPI_receiveData(EUSCI_A3_MODULE);	//Was B0
...
void send_initial_clock_train(void)
{
    ...
    for(i = 0 ; i < 10 ; i++)
    {
        ...
        ui32Dat = SPI_receiveData(EUSCI_A3_MODULE);  //was B0
